...
**Scope-discipline patterns govern Compass's own scope at framework design time** (release planning, codification decisions, roadmap deferrals), distinct from the workflow-execution shapes. **Architecture-discipline patterns govern the structural composition of agents / workflows / hosts**, introduced in v0.3.14. **Observability patterns make framework state visible** — what costs are accumulating, what discipline corrections the user is catching, what structural constraints are being violated. **Catalog now spans 7 shapes / 15 patterns** (post-v0.3.22): enforcement (4) · interaction (1) · freshness (1) · **observability (3)** · handoff (2) · scope-discipline (2) · architecture-discipline (2). Workflow-execution patterns total 11; scope-discipline patterns total 2; architecture-discipline patterns total 2. **The scope-discipline shape is structurally validated** — v0.3.9 introduced with `[declare-not-implement]`; v0.3.10 grew to 2 members with `[hard-line-declaration]`. **The architecture-discipline shape is structurally validated** — v0.3.14 introduced with `[agent-as-surface-independent-unit]`; v0.3.17 grew to 2 members with `[fractal-retro]`. **The observability shape is now durably validated as the largest non-enforcement class** — v0.3.4 introduced with `[role-boundary]`; v0.3.19 grew to 2 members with `[user-as-load-bearing-oversight]`; v0.3.22 grew to 3 members with `[agent-file-compression]`. Observability now matches enforcement on member-count growth: 4 vs 3 members, both with strong structural justification.
