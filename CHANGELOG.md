...
> **External packages now require Architect empirical approval — Engineer can no longer pull a dependency on its own authority.** Closes the build-time hole left open by the v0.2.4 foundational-stack deviation gate: that gate catches *major* dependencies at architecture time; this one covers **every** external package (including small utilities below the "major dependency" line) at build time. The Engineer **refuses-and-escalates** before any `pnpm add` / `npm i` / `pip install` / `cargo add` / `go get` of a NEW package; the Architect runs a **6-category empirical evaluation** (necessity/duplication · maintenance health · security posture · license · transitive footprint/cost · provenance — cite-or-mark-`n/a` per principle #15, so **rubber-stamping fails**) and records the decision as a DRI Decision + a `compass/config.yaml` `dependency_policy.approved_packages` ledger entry. The scanner's new `BUILD-08` checks the manifest against that ledger. Named anti-pattern: **`unvetted-dependency`**. The "empirical" requirement and the **any-package** (not just major) scope both come straight from the user-surfaced gap. Counter ticks to #34 (1 of 5 remaining before Retro #007).

### Added

- **`[external-package-gate]`** registered in `compass/framework/canon.md` — an application of `[refuse-escalate]` (#16) + cite-or-mark-`n/a` (#15) + `[soft-spec-hardening]` (#14). Engineer cannot add a NEW external package without Architect empirical approval; dependencies are an Architect-owned upstream decision. 1-instance codification candidate (build-time complement to an architecture-time gate); names the `unvetted-dependency` anti-pattern.
- **`compass/roles/architect.md` → "External-package empirical approval"** section — the **6-category empirical evaluation** (necessity/duplication · maintenance health · security posture · license compatibility · transitive footprint/cost · provenance/adoption), each cite-or-mark-`n/a`-with-reason; the deviation-gate-vs-package-gate threshold; dual recording (DRI Decision + ledger entry). New role-trigger + DoD assertion + `rubber-stamping` anti-pattern.
- **`compass/config.yaml` `dependency_policy:` block** — `require_architect_approval: true` (hard gate) + `approved_packages: []` (approval ledger with documented entry shape). Canonical approval record + the data the scanner checks (principle #6, configuration as data).
- **`compass/workflows/scan.md` `BUILD-08`** — Unvetted external dependency (manifest/lockfile package absent from the ledger and not grandfathered). High severity; suppressible only with the Architect's recorded 6-category approval as rationale (principle #14 mechanical-verification element).

### Changed

- **`compass/agents/engineer.md`** — `[external-package-gate]` inlined as a core principle; new work-step **4a** (stop-and-escalate before any new install); new refusal rule; new DoD item ("No unvetted dependency"); `unvetted-dependency` added to anti-patterns; DRI note that new packages are not an Engineer decision.
- **`compass/workflows/build.md` Phase 2** — external-package gate added as a load-bearing step body (Engineer escalates → Architect 6-category eval → approve-with-ledger-entry or reject-with-alternative → install only after recording), with the `unvetted-dependency` anti-pattern + principle-#14 framing. "Discipline always" note that the gate holds under deadline pressure.
- **`compass/workflows/create-bet-architecture.md` step 7** — scope note connecting the (major-dependency, architecture-time) deviation gate to the (every-package, build-time) package gate so they read as complements.
- **`compass/workflows/ops.md` Phase 3** — note that a NEW package introduced via `/ops` takes the same empirical-approval path; only upgrades of already-approved packages stay on the normal dependency-upgrade path (no hygiene fast path for new deps).
- **`AGENTS.md`** — principle #15 instance list gains the Architect 6-category external-package evaluation; principle #16 instance list gains the Engineer-install → Architect-empirical-approval escalation.

### Not changed (deliberate)

- **Gate is a hard rule, not a configurable opt-out.** `require_architect_approval: true` mirrors the foundational-stack deviation gate and the reviewer model-split — discipline holds; disabling it is a logged DRI decision, not a silent config flip.
- **Transitive / lockfile-only additions** stay out of scope this round — the gate covers *direct* package additions. Deeper transitive supply-chain defense (lockfile diff review, `npm audit`/`pip-audit` in CI) is noted in the improvements.md watch-for, not built here.
