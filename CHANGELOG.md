...
- **Work branches are cut fresh from `main`, never stacked on a leftover branch (#143).** `_ensure_work_branch` reused *any* current non-main branch — so when a prior run left the repo on a feature branch, the next fix **stacked on it**, carrying that branch's (already-merged) commits into the new PR → **merge conflicts + review scope-creep** (live: PR #116 conflicted because an accounts fix stacked on the merged welcome-back branch, and the reviewer even flagged an a11y "regression" that rode in on those commits). Now: reuse the branch only if it's the *requested* name (resume); otherwise create it from a **fetched `origin/main`** (falling through `origin/master`→`main`→`master`, then current HEAD if there's no remote). +3 tests.

- **`claude-code` is always tool-capable — agents execute, not plan (#141).** The real reason automation/tech-writer/pm steps "planned" (#139 couldn't fix it alone): the router gated the tool-capable path on the agent declaring `executor_tools:`, which only `engineer` does — so those agents dispatched as **tool-LESS single-shots** (no cwd, no `--add-dir`, no permissions, no execute directive) and literally couldn't touch files ("I need read access"). Claude Code *always* has its own Read/Edit/Bash, so the host now **always runs tool-capable + in the repo** when a `project_dir` is set, regardless of `executor_tools:`. Combined with #139's directive, every claude-code step now executes in the project. (The `executor_tools:` gate still governs the metered API host, which must be granted tools explicitly.)

- **Dashboard run logs are live, not end-loaded (#140).** #133 captured a per-run log, but the spawned child **block-buffered** its stdout (output to a file, not a tty) → the log stayed **empty until the buffer filled (~64KB) or the run exited** (live: a 0-byte log while the run was at step 3). The cockpit now launches runs with `python -u` (unbuffered), so the log streams in real time.
...
- **`[reproduce-before-diagnose]` codified + wired into `/fix` (#144).** Compass-original #25 (10th enforcement member), from a vivid consumer post-mortem: a home-app `/fix` (PR #116) diagnosed a *freshness* bug and built ~500 lines of correct-but-irrelevant machinery, when the real fix was a one-field UI change (Plaid `current`, which excludes pending, → `available`, which includes the pending payroll deposit). The delta **equalled the pending deposit** — the single most diagnostic fact, never captured because the orchestrator reasoned about the pipeline instead of looking at the values, then anchored on a true-but-latent staleness defect and wrote a self-confirming test ("the refresh event is sent"). Principle: **for a wrong-value bug, capture observed value + source-of-truth + delta from a real repro *before* theorizing; enumerate candidate causes (stale/wrong-field/wrong-calc) and kill each with evidence; the regression test must reproduce the user's symptom, not the machinery the fix builds; a fix that dwarfs the symptom is a wrong-layer smell.** Four named anti-patterns: `theorize-from-code`, `anchor-on-true-but-irrelevant`, `self-confirming-test`, `wrong-layer-when-fix-dwarfs-symptom`. Wired into `engineer.triage-and-fix` (gate + symptom-test/proportionality postcondition) and `reviewer.review-pr` (right-layer + symptom-test check). Catalog → 25 patterns / enforcement (10).
...
- **Dashboard action guards — confirm, disable-on-submit, already-actioned (#122).** Live use of #119 surfaced a UX gap: clicking a route button gave no feedback, so the operator clicked again (and again) → duplicate resumes. Now every Launch / approve / route form **`confirm()`s before acting**, and on submit **`_act` disables every button on the page** (+ a "submitting…" note) so a gate can't be double-fired while the page waits to refresh. Server-side, `POST /decide` checks the run still has an open gate via `_gate_already_actioned` — if it was already decided (e.g. a stale tab), it returns **409 "already actioned"** with a link back, instead of spawning a second resume. Pairs with #121 (which makes the gate actually clear after one decision). +2 tests.

- **`--run-id` — a resumed gate continues the same run (#121).** `run_id` was minted fresh per invocation (`workflow--bet--timestamp`), so a dashboard `/decide` (a `--from-step` resume) **forked a new run** — the paused run never left the cockpit's ⏸ awaiting queue and the resume showed as a duplicate "completed" row (caught live: one triage gate, three `handed off to /fix` rows, original still awaiting). New `--run-id` makes a resume **continue the original run** (its gate-decision + handoff/`RUN_END` land under the same id → it clears from awaiting and appears once in Done). The cockpit's `/decide` threads the paused run's id through (validated); plain CLI runs still mint a fresh id. +3 tests.

- **`claude-code` host — subscription-backed CLI dispatch (#120).** A fourth host that shells out to the **logged-in `claude` CLI** (`claude -p`, print mode) instead of the metered Anthropic API — **no `ANTHROPIC_API_KEY`, flat marginal cost.** Closes the dashboard-as-orchestrator arc (#118 async gates → #119 action endpoints → **#120 flat-cost dispatch**), so launching workflows from the cockpit no longer burns API credits. **Opt-in** via `--claude-cli` or `COMPASS_CLAUDE_HOST=cli` (the env var is how the dashboard opts in — the #119 cockpit spawns runs with `env=os.environ.copy()`, so exporting it before `cockpit --serve --allow-actions` makes every browser launch flat-cost, **no cockpit change**). The opt-in **remaps only the `claude` host → `claude-code`** (before `select_host`, so no API key is needed); **reviewer steps (Codex/Gemini) are untouched**, preserving cross-model review independence. `--allow-write` maps to `--permission-mode bypassPermissions` (parity with the API host, which already grants write_file + sandboxed bash under allow_write); read-only steps use `default` mode. Claude Code owns its own tool loop (we don't pass our schemas — it's governed by `--add-dir` + the permission mode). **Flat-cost accounting:** the adapter emits a NOTE (tokens + the CLI-reported API-equivalent cost, labeled "subscription, $0 marginal") and **no `usage` event**, so `--max-cost` never false-trips and the cockpit 💰 Spend honestly shows $0 — zero changes to `events.py`/`cockpit.py`. **The flat-cost guarantee:** the adapter **strips `ANTHROPIC_API_KEY` (+ `ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_BASE_URL`) from the subprocess env** — the `claude` CLI prefers an env API key over the subscription login, so leaving it set would bill (and inherit the usage cap of) the metered API, defeating the entire host (caught live: a capped key surfaced a `400 usage limit` instead of using the subscription). On a CLI failure the error now surfaces **stdout** (where the CLI reports usage-limit/auth errors as JSON, often with an empty stderr) instead of a useless "(no stderr)". Pure, injectable-`runner` adapter (`_build_cli_argv`/`_parse_result`/`_subscription_env` unit-tested; the dispatch path tested with a fake runner — no test invokes the real CLI). +22 tests (203 total).

- **Action endpoints — launch + approve from the browser (#119).** The `--serve` cockpit gains two POST endpoints so you **orchestrate from the dashboard** (VISION step 3, slice 2 of 3): **`POST /run`** launches a workflow and **`POST /decide`** approves/rejects/routes a paused gate. The page (with actions on) renders a **🚀 Launch** form (workflow `<select>` from the `compass/workflows/*.md` allow-list, project-dir, context, optional bet, an `--allow-write` checkbox) and **per-awaiting-gate buttons** (approve/reject for an approval gate, one button per route label for a routing gate). **Opt-in: `--serve --allow-actions`** — plain `--serve` stays read-only (POST → 403), mirroring `--allow-write`. The server **only relays to `compass-run`** — never decides: it spawns via an **argv list** (`subprocess.Popen`, never `shell=True`), always injecting `--non-interactive` + the server's `--max-cost`, so **the mechanical gate floor + budget cap still fire** (a dashboard click can't bypass control or run away on cost). Localhost-only (`127.0.0.1`); inputs validated (known workflow name + existing project-dir + numeric step). The spine now carries `project_dir` on `RUN_START` so `/decide` can target any run; `fold_runs` captures it. Pure `_build_run_argv` (tested without spawning). +10 tests (181 total). Next slice: **#120 claude-code host** (subscription-backed dispatch → flat-cost dashboard runs).
...
- **Cost-control batch — orchestrator was burning ~$20 in a few runs (#115 + #116 + #117).** Root cause: every Claude step defaulted to **Opus** (~5× Sonnet), with no spend ceiling and a growing raw-context per step.
  - **#115 — Sonnet by default.** `DEFAULT_MODELS["claude"]` → `claude-sonnet-4-6` (was Opus). New `model_tier:` agent frontmatter (`deep` → frontier/Opus via `router.deep_model`) + the existing `--model` / `COMPASS_MODEL_CLAUDE` overrides. No agent ships `deep` (max savings); bump a hard fix with `--model claude-opus-4-8`. Reviewer/security-reviewer are on Codex/Gemini, unaffected.
  - **#116 — `--max-cost <USD>` budget cap.** Pricing moved into `events.py` (single source for the #106 cockpit rollup + this cap; cockpit re-imports). The per-run sink accumulates spend on every `usage` event and **raises `BudgetExceeded` mid-run** (propagates out of the tool loop → clean halt + `--from-step` resume hint). Per-step `[cost] run ~$x / $cap` line. **Also fixed a latent #104 wiring bug:** run.py passed an `emit(type, **fields)` function as `on_event`, but the tool loop calls `on_event(dict)` — so tool/`usage` events were mangled (never rendered, never priced → cockpit cost showed nothing from real runs). The sink now takes a dict; tool-streaming + usage telemetry actually reach the spine.
  - **#117 — condensed inter-step context.** `_build_user_message` now passes each prior step's **structured summary** (TL;DR + files + next-command, via `parse_step_output`) instead of a 3000-char raw slice — big token cut on later steps.
  - Net: Sonnet (~5×) + condensed context + a hard ceiling → "$20 by accident" becomes "Sonnet by default, capped per run." +8 tests (168 total). Origin: live home-app runs.

### Added

- **`sync-into-consumer.py` — one-command safe framework sync (#114).** A consumer's *embedded* Compass copy (the `compass/` + `.claude/skills` the interactive VS Code `/skills` read) drifts behind the live framework, even when the orchestrator stays current via `--compass-dir`. New `compass/scripts/sync-into-consumer.py <consumer> [--apply]` syncs it safely: **dry-run by default** (prints the exact plan, writes nothing), `--apply` performs it after **auto-backing-up** `consumer/compass/` → `.compass-backups/<ts>/`. Explicit policy — **OVERWRITE** the framework machinery (`compass/{agents,workflows,framework,templates,scripts,orchestrator}`, `AGENTS.md`, `CLAUDE.md`, `.claude/skills`, `.codex/prompts`), **PRESERVE** the consumer's own (`config.yaml`, `roles/`, `docs/`, `PROJECT.md`, `README.md`, `.claude/settings*`, `.codex/config.toml`, `.github/`, `.mcp.json` — the script only ever writes overwrite-set paths), **PRUNE** the framework's own meta-logs from the copy (`improvements.md`, `retros/`). Pure `plan_sync` + `apply_plan` (testable). MIGRATION.md + scripts/README.md updated. +9 tests (160 total). Origin: home-app's embedded docs went stale while running both VS Code `/skills` and the orchestrator.
...
- **Right-size the path to the work — the enhancement lanes (#109, v0.3.51).** The front door used to send **every** `enhancement`/`problem` to `/create-brief` (a new bet + the full pipeline), even when the work was a slice of an existing bet or trivial — the planning-side twin of the `/fix` ITIL ceremony. Now `support.classify-intake` **right-sizes**: a *new capability* → `/create-brief` · a *slice of an existing bet* → `/create-story --bet <id>` (no new brief) · *trivial* → hygiene. **The classifier names the specific bet** — the orchestrator injects a compact **bet catalog** (each `docs/bets/*/brief.md`'s id + type + status + one-liner) into any agent step that declares `loads_bet_catalog: true` (new `run.py` helpers `_load_bet_catalog` + `_reads_bet_catalog`; `support.md` opts in), so a slice gets `--bet <matched id>` by name, not a blank. **Mechanical guard:** `/create-brief` now refuses to mint a redundant bet for a slice and points to `/create-story`. The 7-ITIL routing gate is unchanged (`enhancement → /create-brief` stays the default; right-sizing rides in the recommendation + the guard). Lane 3 (a brief-less lightweight build for trivial enhancements) is **declared**, not yet wired. 1st instance of candidate `[right-size-the-path-to-the-work]` (canon codification DRI-gated); pairs with `[ai-collapses-org-tiering]`. +3 tests (142 total). Origin: DRI asked whether every enhancement should go through the same heavyweight space.
- **`/fix` ITIL-tier collapse (#108, v0.3.50; implements the Retro #022 redesign).** Removed the repo-blind `support.triage-bug` step + its "triage confirmed" HITL gate from `/fix` — in the AI world the L1/L2→L3 escalation is ceremony (the "expensive engineer you protect" is an agent that reads the code instantly, so a support tier that interrogates the user then escalates adds nothing). `engineer.fix-bug` is renamed **`engineer.triage-and-fix`** and now owns reproduce-from-code + diagnose + severity itself (used by both `/fix` and `/triage`'s needs-fix branch). `/fix` goes **8 steps → 6**, one HITL gate (merge). **Kept (not tiering):** front-door routing (`/triage`), **maker ≠ checker** (different-model Reviewer + Security Reviewer + response loop — the fixer validates by running tests but never reviews its own diff), and the HITL gate. **`[refuse-escalate]` refined at the application level** (reproduce from code first; ask the human only for what code can't reveal — prod data, account state, credentials; halt if still irreproducible) — Principle #16 itself (foundational scope-widening) untouched. `support` shrinks to the front door (`fix` removed from its `participates_in_workflows`). Nothing else dropped: regression-test-first, MOV, per-surface-vertical-test, contract sweep, no-hotfix-exception, promote-to-tech-debt-brief all preserved. 1st instance of candidate `[ai-collapses-org-tiering]` (canon codification deferred — DRI-gated). +tests updated (139 total). Origin: live home-app run where repo-blind triage interrogated the DRI for facts the code answers.
...
- **Claude prompt caching + usage telemetry (#105, v0.4.0-alpha-16).** The tool loop in `hosts/claude.py` re-sent the full agent-file system prompt + tool schemas uncached on every iteration — the dominant API cost. Now the static prefix is cache-marked (`system` → a `cache_control` text block; the last tool schema carries `cache_control`) and a single **rolling conversation breakpoint** rides the last message each turn so the growing prefix (prior tool results / file reads) is cached incrementally — exactly one rolling breakpoint is kept (system + tools + 1 ≤ Anthropic's 4-breakpoint cap). ~10% cost on cache reads, 5-min TTL covers a tool loop + back-to-back steps using the same agent; below-minimum prompts ignore `cache_control` harmlessly. Caching is Claude-specific by design (host adapter; OpenAI auto-caches, Gemini explicit — out of scope). Cost is now **observable**: each call's `response.usage` (input/output/cache-read/cache-creation tokens) is emitted as a `usage` event on the #104 spine (best-effort, guarded — `on_event` also threaded into single-shot `dispatch`). +5 tests (133 total). Follow-on: cockpit $-rollup from the usage events.
- **Event spine + portfolio cockpit (#104, v0.4.0-alpha-15; delivery layer slice 1, VISION step 3).** #97 added an `on_event` tool-loop sink but never threaded it past the loop and emitted no run/step/gate lifecycle. #104 completes the spine: new `compass/orchestrator/events.py` (event types + `make_event` + `terminal_sink`/`jsonl_sink`/`multi_sink` + `load_events`); `run.py` emits `run_start · step_start · gate_open · gate_decision · handoff · step_end · run_end` through one `emit` closure and passes it as `on_event` into `dispatch_to_host` → `dispatch_with_tools` (so tool events share the spine); `router.py` forwards `on_event`; `claude.py`'s `_default_tool_event` re-homed to `events.terminal_sink`. The durable store is **user-local and portfolio-wide** — `~/.compass/orchestrator/events.jsonl` (`$COMPASS_HOME` override), deliberately NOT in the project repo (live telemetry spanning all projects; keeps git clean; no concurrent-worktree collision per #102). The in-repo `runs.jsonl`/`hitl.jsonl` stay the per-project audit. First consumer: **`python3 -m compass.orchestrator.cockpit`** (`cockpit.py`) — a portfolio-wide text view: **⏸ Awaiting your decision** (open gates across every project, each with a ready-to-run `--approve`/`--reject` command), **▶ In flight**, **✓ Done/halted**. `--no-events` opt-out; `--dry-run` skips the durable sink. +14 tests (128 total). Convention candidate `[telemetry-user-local-not-in-repo]` (1 instance). Next slices: HTML `/dashboard` live tab · Slack/WhatsApp sinks · inline approve-from-cockpit.
- **Front-door `/triage` ITIL intake router BUILT (#103, v0.4.0-alpha-14; implements declared #98).** `/triage` is now the front door for every incoming item. New `support.classify-intake` (Step 1) proposes an ITIL category — incident / bug / enhancement / problem / change / service-request / not-an-issue — by observed impact+urgency; an intake **routing gate** (Step 2) routes it: `incident` continues inline through the incident branch, while `bug`→`/fix`, `enhancement`/`problem`→`/create-brief`, `change`/`service-request`→`/ops` **hand off cross-workflow** and `not-an-issue` closes. This is the **2nd `[conditional-dispatch]` instance** (cross-workflow), so the pattern is **codified to canon (v0.3.49; 4th architecture-discipline member; catalog → 23 patterns)**. Engine change: route targets generalized from `int` (inline step, #96) to `int | str` — a `str` target is a `/workflow` hand-off or `close`, which logs the decision, prints the recommended next command, and ends the run (`break`). The category→workflow mapping lives in the dispatch graph's Routes block, never hardcoded in the executor. **v1 = hand-off (recommend the command); auto-chaining the child workflow is v2** (#87 surface 3). `--dry-run` now renders routing gates with their routes. +7 tests (114 total). Origin: two live home-app `/triage` runs that classified correctly but had nowhere to route (the dead-end behind #98).
