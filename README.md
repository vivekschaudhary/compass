> **Status:** Early alpha. Used by the author on real projects. Orchestrator v0.4-alpha ships (exact alpha in CHANGELOG) — clone + set `ANTHROPIC_API_KEY` + run `python3 -m compass.orchestrator.run setup-product`. Feedback welcome via Discussions; no support promises.
