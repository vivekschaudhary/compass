...
- **Agents own tasks; workflows sequence agents.** 17 agent files in `compass/agents/` (**14 coded** by v0.3.36; **`gtm`, `sre`, `product-owner` declared** v1.0, not yet coded). Each is self-sufficient: identity + inlined principles + tools + task definitions (gate/work/postcondition) + refusal rules + handoffs. Workflow files in `compass/workflows/` are **thin dispatch graphs** that sequence `<agent>.<task>` references — methodology lives in the agent task definitions, not the workflow.
