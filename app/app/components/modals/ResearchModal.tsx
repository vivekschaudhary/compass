...
// as an editable playbook (PlaybookEditor): edit, add, then promote to tracked tasks.
export function ResearchModal({ open, onClose, engagementId, actor, onDone, storyList, providerLabel }: {
  ...
  useEffect(() => {
    if (open) { setStory(firstTicketId(storyList, "researcher")); setErr(null); setUrl(null); setLog(""); setActions(null); }
  }, [open, storyList]);

  async function run() {
    if (!story) { setErr("Pick a research ticket to run."); return; }
    setBusy(true); setErr(null); setUrl(null); setLog(""); setActions(null);
    try {
  ...
  return (
    <Modal open={open} onClose={onClose} busy={busy} maxWidth={done ? 540 : 460} title="Run research" subtitle={`Reads the product brief and drafts a research doc into ${providerLabel} — then turns the recommendations into a playbook.`}>
      {!done && (
