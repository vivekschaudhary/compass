...
export async function storyFor(taskId: string): Promise<string | null> {
  ...
  const { data: run } = await sb.from("workflow_run")
    .select("subject_key, subject_ref").eq("id", task.workflow_run_id).maybeSingle();
  // The TRACKER's key, never the agent's ref. `--story` is passed to a process that looks the issue
  // up in Jira; handing it `E1-S3` would send it looking for an issue that does not exist.
  return (run?.subject_key as string | null) ?? null;
}
