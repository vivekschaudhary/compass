...
**Work:** list bets shipped/measuring this window (1-line + brief link) → bets carried forward (aggregate) → hygiene aggregate → incidents resolved (link to postmortem) → draft → **HITL gate: do NOT publish**; tell user: _"Sprint comms drafted at <path>. Review and publish to <channel> when ready."_
