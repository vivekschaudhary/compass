...
**Work:**
1. **Triage from the code, not the user.** Read + run the actual source via tools (`executor_tools`, #87) to **reproduce** the defect — don't interrogate the user for what the code shows. Classify **severity** (P0: prod down/data loss/security · P1: major feature broken · P2: degraded · P3: minor/cosmetic), classify **defect vs enhancement** (a **defect** = something is broken → Jira **Bug**; an **enhancement** = new/changed behaviour that arrived as a `/fix` → Jira **Story**, opened directly, no brief), and check for **duplicates** (issue tracker + code). **Apply `[refuse-escalate]` (refined for a code-capable agent):** reproduce from the code first; ask the human ONLY for what code genuinely can't reveal (prod data, account-specific state, credentials); if you still cannot reproduce after exhausting code-level repro, **state exactly what you need and HALT — do not fix blind, do not escalate noise.**
   - **Wrong-value / wrong-output bugs need the VALUE, not just the pipeline (`[reproduce-before-diagnose]`).** Capture the **observed value, the source-of-truth value, and the delta** from a real reproduction — the delta usually names the cause (a gap that equals a pending deposit ⇒ a `current`-vs-`available` field bug, NOT a freshness bug). The actual numbers are exactly what reading code *can't* reveal, so they are a legitimate — often the only — thing to ask the human for; "triage from the code" does not mean theorize from the code. **Enumerate the candidate causes — stale-over-time · wrong-field · wrong-calculation — and eliminate each with evidence. Never resolve an ambiguous symptom to the first plausible reading, and never anchor on a true-but-unrelated defect found along the way** (it feels like progress but fixes nothing the user reported — the most dangerous kind of finding).
...
3. **Write the failing regression test FIRST** (first commit: `test: reproduce <bug>`) — it must **reproduce the user's observable symptom** (the wrong value/output as the user sees it), not the machinery you're about to build. A test that asserts your own new mechanism fired (e.g. "the refresh event is sent") is **self-confirming** — it passes whether or not the user's number is ever right, so it can't fail for the actual bug. **If you can't express the symptom as a test, you haven't found the bug yet.**
4. Implement the **minimum** fix (subsequent commits: `fix: <description>`) — no scope creep beyond the defect. **Proportionality check: if the fix balloons far past the symptom (a one-field bug → a new endpoint + polling + timeout), STOP — that size mismatch is a wrong-layer smell; re-examine the diagnosis (and the delta) before building.**
...
9. **Deliver the work — don't leave it on the floor (#145).** **Commit every code change** (no uncommitted source/test files left behind) and **push the branch**. Put a short **triage summary** (severity · repro steps · root-cause vs symptom + affected bet(s)) in your output — it becomes the PR body. **Do NOT open the PR yourself (#92):** after you commit + push, the **orchestrator** runs the CI-parity check suite (lint/typecheck/test/build) in the worktree and **opens the PR only if they pass** — so no dirty PR is ever created and "checks green" is verified, not self-attested. If you genuinely can't commit or push (no git remote, `gh`/git unavailable, auth), say so with a leading `REFUSE: <what's blocking delivery>` — never report success with the change uncommitted. Halt for Reviewer.
10. **Classify defect vs enhancement in your triage summary (#71).** State the classification plainly — a **defect** (something broke) or an **enhancement** (new/changed behaviour) — e.g. a line `CLASSIFICATION: enhancement` when it's not a defect (defect is the default). The **orchestrator** reads this + your PR and mechanically **writes the fix record + projects it to Jira** as a **Bug** (defect) / **Story** (enhancement) at run completion — you do NOT write the record or call Jira yourself (that projection is orchestrator-owned so it can't be skipped, #89).
**Postcondition:** defect reproduced from the code (or HALTED with a precise ask if irreproducible) · **for a wrong-value bug: observed/expected/delta recorded in the triage summary and candidate causes (stale/wrong-field/wrong-calc) eliminated with evidence** · severity classified + repro documented in the PR · **failing regression test reproduces the user's observable symptom (not the built mechanism — a self-confirming test fails the gate)**, landed BEFORE the fix (visible in commit order) · minimum fix **proportional to the symptom (a fix that dwarfs the symptom is re-diagnosed or escalated as a possible wrong-layer root, not shipped as-is)** · all checks run locally for fast feedback (but **'checks green' is orchestrator-verified, not self-attested — #92**) · runtime artifact inspected · **the change is COMMITTED and PUSHED (#145); the orchestrator runs the CI-parity checks and opens the PR on green (#92) — uncommitted/unpushed work = task NOT complete; if delivery is blocked, `REFUSE:` rather than report success** · own diff NOT self-reviewed (a different model reviews — validation ≠ review) · **defect-vs-enhancement stated in the triage summary so the orchestrator projects the fix record to Jira as a Bug/Story (#71) — the projection is orchestrator-owned, not yours to write** · if a deeper architectural root is suspected, ship the symptom fix AND escalate to a `/create-brief` tech-debt bet (don't accumulate silent symptom fixes).
