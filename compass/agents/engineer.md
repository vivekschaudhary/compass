...
9. **Deliver the work — don't leave it on the floor (#145).** **Commit every code change** (no uncommitted source/test files left behind) and **open the PR** with a short **triage summary** (severity · repro steps · root-cause vs symptom) + affected bet(s). Uncommitted work or a missing PR is the #1 way a "successful" run ships nothing (no commit → no PR → no deploy). If you genuinely can't commit or open the PR (no git remote, `gh` unavailable, auth), say so with a leading `REFUSE: <what's blocking delivery>` — never report success with the change uncommitted. Halt for Reviewer.
10. **Track the work as a Jira item (#71).** Write a **fix record** (`compass/templates/fix.md`) at `docs/fixes/<id>.md` (hygiene) or `docs/bets/<bet-id>/fixes/<id>.md` (bet-linked), carrying `type: bug` (defect) or `type: enhancement`, the severity, and the **PR link**. It **projects to Jira** as a **Bug** (defect) or **Story** (enhancement) — **bet-linked → under the bet's Epic**, hygiene → standalone in the backlog. The record holds the `jira_key` pointer (idempotent re-push); its **status derives from the PR** (#57), not a hand-set field. No Jira credentials → the fix record still lands as the Compass-primary cache with an honest filesystem-fallback label.
**Postcondition:** defect reproduced from the code (or HALTED with a precise ask if irreproducible) · **for a wrong-value bug: observed/expected/delta recorded in the triage summary and candidate causes (stale/wrong-field/wrong-calc) eliminated with evidence** · severity classified + repro documented in the PR · **failing regression test reproduces the user's observable symptom (not the built mechanism — a self-confirming test fails the gate)**, landed BEFORE the fix (visible in commit order) · minimum fix **proportional to the symptom (a fix that dwarfs the symptom is re-diagnosed or escalated as a possible wrong-layer root, not shipped as-is)** · all checks green · runtime artifact inspected · **the change is COMMITTED and a PR is OPEN (#145) — uncommitted work or no PR = task NOT complete, not "done"; if delivery is blocked, `REFUSE:` rather than report success** · own diff NOT self-reviewed (a different model reviews — validation ≠ review) · **defect-vs-enhancement classified and a fix record projected to Jira as a Bug (defect) / Story (enhancement), bet-linked under the Epic or standalone, linking the PR (#71)** · if a deeper architectural root is suspected, ship the symptom fix AND escalate to a `/create-brief` tech-debt bet (don't accumulate silent symptom fixes).
