...
- **The cockpit already exists in pieces — it's split.** `/plan` is the schedule half (in flight · blocked · next up · done). `/status` is the decisions half (per-item `Awaiting` column + an "Awaiting human approval" list + Blockers). `/dashboard` merges both into one browser page. **The vision is to elevate `/status` into the cockpit:** promote "Awaiting your decision" to the top, make it **actionable** (approve from the list, not just see it), feed it **live** from the orchestrator's open gates, and span it **across all in-flight tasks** — with `/plan` as the schedule feeder.
...
3. **The cockpit** — one live, actionable, portfolio-wide view of what's moving + what needs me.
