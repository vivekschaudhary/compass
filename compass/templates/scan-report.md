...
> Continuous quality scanner output. Findings, not failures. Re-render with `/scan <epic-id>`. Never hand-edited — the next `/scan` run will overwrite. Owners triage; the scanner informs.
...
- **Severity:** Critical
- **Confidence:** High
- **Location:** `docs/epics/<BET-ID>/brief.md` (`status: proposed`)
...
- **Severity:** High
- **Confidence:** Medium
- **Location:** `docs/epics/<BET-ID>/stories/<story-id>/story.md` AC items vs. test files in `e2e/`
...
- **Severity:** Critical
- **Confidence:** High
- **Location:** `docs/epics/<BET-ID>/ops/*` DRI logs
...
- **Severity:** High
- **Confidence:** Medium
- **Location:** `docs/epics/<BET-ID>/runbook.md`
...
|---------|----------|---------------|------|-----------|----------|
| <Title> | <sev> | <name> | YYYY-MM-DD | <justification> | `docs/epics/<BET-ID>/brief.md#DRI` |
