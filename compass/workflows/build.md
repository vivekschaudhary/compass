...
6. **Engineer implements:**
   ...
   **External-package gate (load-bearing).** Per `[external-package-gate]` (canon.md) + principle #16 (refuse-escalate). If implementing the story requires a NEW external package — anything not already in the project manifest and not already on `compass/config.yaml` `dependency_policy.approved_packages` — **the Engineer does NOT install it.** Engineer **STOPS** and escalates to the Architect with: package name + version range, the need it serves, and whether an already-approved dependency could cover it. The Architect runs the **6-category empirical evaluation** (`compass/roles/architect.md` → "External-package empirical approval": necessity/duplication · maintenance health · security posture · license · transitive footprint/cost · provenance) and either **approves** — recording a DRI Decision + a `dependency_policy.approved_packages` ledger entry — or **rejects** with an alternative. The install happens only after approval is recorded. This is the **build-time complement** to the `/create-bet-architecture` step-7 deviation gate: that gate routes *major* dependencies (tools/services/frameworks/runtimes/data stores) to a foundational amend; this gate covers **every** external package, including the small utilities below the "major dependency" line. Nothing enters the dependency tree unevaluated.

   **Named anti-pattern: `unvetted-dependency`.** Engineer runs `pnpm add <pkg>` / `npm i <pkg>` / `pip install <pkg>` / `cargo add` / `go get` to keep moving, then rationalizes the choice after the fact — or recharacterizes a real dependency as "just a small util, not really a package" to slip the gate. Either way an unevaluated third party enters the supply chain: unknown maintenance health, license, CVE exposure, transitive footprint. **The hardening is the empirical Architect approval before install + the ledger the scanner checks (`BUILD-08`)** — per principle #14, "I'll just add this one package" is the soft spec being rationalized; the fix is the explicit gate + mechanical ledger check, not "Engineer should be careful with dependencies."
...
No shortcuts under pressure. Full Codex review + Architect compliance + Security review (if applicable) on every PR including drafts and hotfixes. **No new external package without Architect empirical approval** — the `unvetted-dependency` gate holds under deadline pressure too; "just one quick package" is exactly when the supply-chain risk slips in.
