...
- **Source or stub present** — user provides ≥1 source (link, free text, or existing stub epic-id with `portfolio_stub: true`). **On failure:** *"Provide a source link, description, or stub epic-id to begin."*
- **Brief not already drafted** — if `docs/epics/<epic-id>/brief.md` exists with `portfolio_stub: false`, refuse: *"Brief already drafted. Use `/create-story <epic-id>` or `/create-epic-architecture <epic-id>` next."*
- **Not a slice of an existing bet** (`[right-size-the-path-to-the-work]`, v0.3.51) — if the request is really a slice/addition to an already-approved bet (check the existing bets under `docs/epics/`), **refuse** and right-size: *"This looks like a slice of bet `<id>`. Run `/create-story <id>` instead of minting a new bet."* Only genuinely new capabilities/hypotheses get a fresh brief. (Enforced in the PM task gate, per `[refuse-escalate]` spirit — don't silently create a redundant bet.)
...
**Input:** source material (link / free text / stub context) · epic-id (if known) · `docs/foundation/product.md` for context
...
**Artifact target:** `docs/epics/<epic-id>/brief.md`
**What it covers:** human reviews `docs/epics/<epic-id>/brief.md` against Verification checklist. Pass → approve: orchestrator runs promote to the Artifact target with `status: approved` automatically; interactive sessions flip `status: proposed` → `status: approved` (or `--approve` CLI) + commit. Fail → reject and re-dispatch PM. **Principle #16:** PM must NOT self-approve.
...
**What it covers:** confirm brief approved · update bet tracking · surface next recommended workflow (`/create-epic-architecture <epic-id>` if `architecture_required: true/auto`, else `/create-story <epic-id>`).
**Output:** bet status current in delivery tracking

## Workflow-level verification (final GATE)

- [ ] `docs/epics/<epic-id>/brief.md` exists with `status: approved`
...
**TL;DR** (3 bullets) · **Files created/modified** · **Next recommended command** (`/create-epic-architecture <epic-id>` or `/create-story <epic-id>`) · **Open questions/risks**.

## Notes

**Promote-stub mode:** when `/create-epics` ran first, it creates stubs with `portfolio_stub: true`. Promoting fills the full brief content while preserving the bet's portfolio context (parent trace, wedge position, hypothesis lineage). Do NOT re-derive the hypothesis — keep it from the stub.
