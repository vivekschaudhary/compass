...
**Output:** PR with regression-test-first commit order + triage summary. At run completion the **orchestrator** projects a fix record (`docs/fixes/<id>.md` or `docs/epics/<bet>/fixes/<id>.md`) to Jira as a **Bug** (defect) / **Story** (enhancement) — bet-linked under the Epic, hygiene standalone, linking the PR; `jira_key` stored (idempotent).
