...
**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **354 pass** (+5: `TestStoryDependencies` — feature blocked by un-delivered design+copy; block clears when both `ready`; human-owned awaiting-delivery is amber; owner/type surface on the node; render shows `(design, human)` + `⛔ blocked by`). consistency-check CONSISTENT.

**Files touched (9):** `compass/templates/story.md` · `compass/templates/pr-template.md` · `compass/agents/pm.md` · `compass/agents/designer.md` · `compass/agents/ux-writer.md` · `compass/agents/engineer.md` · `compass/workflows/create-story.md` (→ v0.3.57) · `compass/orchestrator/wbs.py` · `compass/orchestrator/tests/test_wbs.py` (+ CHANGELOG + this file). Counter: #171. **(#031 batch: #168–#172 of #168–#177.)** On branch `mvp/design-copy-stories` → PR; cross-model review (Codex/Gemini, not Claude). *(Deprecated `compass/roles/{designer,ux-writer}.md` still name the sidecars — grace-period, removed in v0.4; the migrated `compass/agents/` are authoritative.)*
