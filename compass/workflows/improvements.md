...
**Verification:** \`python3 -m unittest discover -s compass/orchestrator/tests\` → **376 pass** (+4: \`_bet_doc_paths\` order; story → jira routing; brief → confluence routing; missing-file ERROR — all via the no-creds fallback path so they're deterministic). consistency-check CONSISTENT. Live demo on home-app WLT-27 routed all 10 artifacts correctly (Confluence for docs, Jira for stories), falling back honestly with no creds set.
