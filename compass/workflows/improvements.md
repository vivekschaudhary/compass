...
- **Retro #028** (improvements #138–#147): [retros/2026-06-26-retro-028-improvements-138-to-147.md](retros/2026-06-26-retro-028-improvements-138-to-147.md) — the "**`/fix` becomes a real pipeline**" batch, **10/10 dogfood-driven** (one continuous live `/fix` on home-app). Removed every reason the non-implementer steps were no-ops: reviewer-gets-diff (#138) · execute-not-plan (#139) · live logs (#140) · always-tool-capable (#141) · `changes ↗` (#142) · branch-fresh (#143) · **`[reproduce-before-diagnose]` codified** (#144, canon #25) · delivery-incomplete loud (#145) · cadence 5→10 (#146) · approve-merge merges (#147). Crossed from "step 1 works" → "the pipeline delivers." Full-surface audit consolidated into Retro #029 (shared current surface). Codification: `[reproduce-before-diagnose]` shipped; `[fail-loud-not-silent]` durable; host-capability-validation 2nd instance.
- **Retro #029** (improvements #148–#157): [retros/2026-06-26-retro-029-improvements-148-to-157.md](retros/2026-06-26-retro-029-improvements-148-to-157.md) — the "**trustworthy, surface-independent, subscription-only control plane**" batch, **10/10 dogfood-driven**; **shipped the first real story (WLT-26-1) to `main`** via claude→codex→PM→human, no API keys. host-isolation (#148) · per-step ✓/✗ (#149) · workflow-aware delivery (#150) · doc-branch-skip (#151) · idle-timeout+streaming (#152) · no-self-approve guard (#153) · awaiting observable+resumable (#154) · **`codex-cli` host** (#155) · approve carries `--bet`+mode (#156) · resume reuses branch + merge gate names next step (#157). `[reproduce-before-diagnose]` practiced 4× (validation within one batch). **Independent audit fixed 2 in-batch** (router error-string omitted `codex-cli`; `__init__.py` stale alpha — both consistency-check blind spots, now closed). **PROMOTE candidate: `[observability-before-trust]`** (≥3 acted-on instances). New candidate `[surface-carries-run-state]` (2).
- **Retro #030** (improvements #158–#167): [retros/2026-06-29-retro-030-improvements-158-to-167.md](retros/2026-06-29-retro-030-improvements-158-to-167.md) — the "**enterprise delivery control tower**" MVP batch: stack-agnostic agents + override layer (#160) · governance audit + actor + cross-model provenance (#161) · SOW-conformance (#162) · Compass→Jira/Confluence one-way projection (#163, #166) · exec WBS + manage-by-exception (#164) · coherence floor / stale-halt (#165) · full-backlog decompose (#158) · authoring-write-default (#159) · **cross-model (Codex) review of the batch found 3 real defects** (#167). Candidates: `[compass-primary-external-projection]` (2) · `[honest-degradation]` · **`[reviewer-scope-separation]` now 2 → promote**.
- **Retro #031** (improvements #168–#177): [retros/2026-06-29-retro-031-improvements-168-to-177.md](retros/2026-06-29-retro-031-improvements-168-to-177.md) — the **live-dogfood batch** (~8/10 from real WLT-26/27 browser runs); first genuinely **parallel, worktree-isolated story builds** end-to-end. design/copy-as-stories (#171) · story-scoped build (#172) · branch-isolation (#173) · worktree-default (#174) · telemetry-out-of-git (#175) · dashboard story-label (#176) · approve refresh-race (#177) + host/CI/idle fixes (#168–#170). **PROMOTE candidates: `[capability-by-default-not-flag]`** (3: #159/#174/#179) · **`[actionability-before-trust]`** (3: #176/#177/#178). Independent context-free audit confirmed the surface clean (counts/sidecars/code-vs-docs); `compass/roles/` refs verified intentional grace-period. Watch-for: run-scope step-artifact path · targeted `--reap-stale --run-id` · cockpit self-reload after a fix merges.

**Batch #029 (#148–#157) COMPLETE. Next framework retro #030 (every-10) covers #158–#167 — fires at #167.** Watch-fors carried: Telegram/Slack surface adapter · `DELIVERY INCOMPLETE` PR-aware fix · `[observability-before-trust]` → canon decision · pull→push gate notification · stranded WLT-26-1 e2e tests.
...
**What shipped (the log-capture slice of #129):** the cockpit now **mints the run id** for `POST /run` (matching run.py's format) — reusing the #121 `--run-id` plumbing — so it can **capture the run's stdout+stderr** to `$COMPASS_HOME/orchestrator/runs/<run_id>.log` (append, line-buffered; `/decide` resumes append to the same file). New **`GET /log?run=<id>`** serves a live, auto-refreshing tail (last 64KB); a **`log ↗`** link sits on each run card in **actionable mode** (`--allow-actions` — read-only cockpit doesn't capture logs). Run-id is regex-validated (no path traversal); localhost-only; the `--run-id` arg now applies to both actions (`_build_run_argv` refactor).
...
### 2026-06-25 — codex-cli host: the Reviewer on your Codex subscription, flat-cost (#155)

**Trigger origin (Principle #19):** the DRI tried to `/build WLT-26-1` and — once past the bet/story-id confusion — hit a wall I diagnosed *before* it halted: the **Reviewer** routes to `[codex, gemini]` (cross-model independence), and a CLI-only operator (claude subscription, no API keys) had **neither reachable** — `OPENAI_API_KEY`/`GEMINI_API_KEY` unset, `gemini` CLI not installed. Every `/build` would halt at review. But the operator **had the `codex` CLI installed + logged in.**

**What shipped:** `codex-cli`, the exact analog of the `claude-code` host (#120), dispatching via `codex exec` on the logged-in Codex subscription — no `OPENAI_API_KEY`, flat marginal cost. `codex ≠ claude`, so running the reviewer here **preserves** the no-same-host-self-review principle (the reason the reviewer excludes claude in the first place). Probed the live `codex exec --json` schema before building (`reproduce-before-diagnose`): `item.completed`/`turn.completed` events + `-o` final-message capture. Mirrors the Claude host's shape — `_build_cli_argv` / `_parse_result` / `_progress_line` (pure, tested) + a streaming idle-timeout runner (#152 pattern) + `_subscription_env` stripping `OPENAI_API_KEY` + `_compose_message` prepending the agent file (codex has no `--append-system-prompt-file`) + the #139/#153 execute-and-don't-self-approve directive. Permission parity: allow_write → `--dangerously-bypass-approvals-and-sandbox` (the reviewer needs `gh` + network) else `-s read-only`; `--ignore-user-config` is the #148 isolation analog. Opt-in `--codex-cli` / `COMPASS_CODEX_HOST=cli` remaps only `codex` → `codex-cli`; router + run.py wired exactly like #120.

**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **285 pass** (+20: argv read-only/bypass/model, `_parse_result` from -o + stream + raises, progress lines, dispatch NOTE-not-usage, OPENAI_API_KEY strip, execute+no-self-approve directive, idle parse, router `_has_key`/`_family`/route, remap). consistency-check CONSISTENT (host-list check passes). **Live smoke** through the real `codex` CLI: streamed `· READY`, returned `READY`, NOTE `codex-cli · $0 (subscription)`; `select_host(['codex-cli','gemini'])` → `codex-cli`.

**Files touched (8):** `compass/orchestrator/hosts/codex_cli.py` (new) · `compass/orchestrator/hosts/router.py` · `compass/orchestrator/run.py` · `compass/orchestrator/tests/test_codex_cli.py` (new) · `AGENTS.md` · `SETUP.md` · `CHANGELOG.md` (+ this file). Counter: #155. **(#029 batch: #148–#157.)** Subscription-only operators can now run the whole loop: implementer on `claude`, reviewer on `codex`, both flat-cost, independence intact.

### 2026-06-25 — dashboard "approve" works on bet-scoped gates: /decide carries --bet + run mode (#156)
...
- **Run mode is now recoverable from the spine.** `run_start` records `claude_cli` / `codex_cli` (alongside the existing `allow_write`); `fold_runs` tracks them per run; `_decide_forms` (the approve/reject form) and `_approve_cmd` (copy-paste) carry them. So a resume keeps the run's write perm and its **subscription CLI hosts** — directly the DRI's "update the codex-cli as well": a resumed review stays on `codex-cli`, not the metered API. (Dashboard spawns also inherit the cockpit env, but a human's copy-paste in a fresh shell wouldn't — now it's explicit.)
...
### 2026-06-26 — /create-story decomposes the full backlog, not one story at a time (#158)
...
**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **298 pass** (+5: authoring defaults on even when caller says False · code workflows keep caller choice · the two sets are disjoint · the 5 live permission-block phrasings classify ✗ · normal completed work isn't false-tripped — all reconstructed from the actual browser log). consistency-check CONSISTENT.

**Files touched (3):** `compass/orchestrator/run.py` · `compass/orchestrator/cockpit.py` · `compass/orchestrator/tests/test_graph.py` (+ CHANGELOG + this file). Counter: #159. **(#030 batch: #158–#159 of #158–#167.)** Recovery for the live case: re-run `/create-brief WLT-27` on home-app — it now writes by default and the WLT-27 brief + research land. Known follow-up: a post-run check that the *expected canonical artifact path* exists (stronger than the phrase-based step heuristic) — fold into the coherence layer.

### 2026-06-28 — stack-agnostic delivery agents + pluggable stack profiles + override layer (#160)

**Trigger origin (DRI direction):** the control-tower MVP plan's capability #1. The DRI caught the design error directly — *"creating agent files with Next/TS runtime contracts is incorrect, we should keep it agnostic; the arch can be standardized but the stack selection should be agnostic"* — plus *"compass should provide an ability to overwrite templates and workflows, but the sample runs a well-oiled industry-standard methodology and template."* Two statements, one principle: **opinionated defaults, fully overridable.** First brick of the MVP (the credibility gate for the .NET/Blazor licensing-portal pilot), and a correction that protects the vendor/stack-neutral thesis at the agent layer.

**The problem.** `engineer.md` / `reviewer.md` / `automation.md` hardcoded Next.js/TS runtime contracts — `.next/*` manifests, `[rsc-prop-serialization]`, `[server-action-file-export-purity]`, the reviewer's entirely-Next.js Step-0 framework-registration check, E2E-detection-from-`package.json`, `@ts-ignore`. Compass therefore only *really* governed the stacks we'd hand-coded; a .NET/Java/Python provider would get a TS-shaped reviewer. The first draft of the plan made it *worse* (add Blazor contracts too = two flavors baked in); the DRI's correction moved the stack OUT of the agent entirely.

**The fix — agents carry methodology, profiles carry the stack.**
- **De-stacked the three delivery agents** to the stack-neutral stance: *"verify your stack's runtime artifact (per the loaded stack profile), check your stack's framework runtime contracts, run the stack's production build, don't suppress compiler errors."* The discipline (`[mechanical-output-verification]`, contract check, production-build, `[cross-artifact-sweep-on-contract-shift]`, `[per-surface-vertical-test]`) stays; the Next.js examples leave. Each gained a *"Stack-agnostic — read the Active stack profile"* anchor principle.
- **Stack profiles** at `compass/stacks/<stack>.md` carry the specifics (build/test cmds · production-build · runtime-artifact inspection list · framework runtime contracts · test-framework detection · public-config & compiler-suppression conventions). Shipped: **`nextjs-ts`** (extracted verbatim from the removed agent content — TS/Next consumers keep identical coverage) + **`dotnet-blazor`** (render modes, `blazor.boot.json` boot-manifest, `[blazor-interop-serialization]`, `[prerender-hydration-state]`, `[render-mode-mismatch]`, bUnit/xUnit detection from `.csproj`, ASP.NET Core authz + EF query filters for the per-surface vertical).
- **Selection + injection:** a `stack:` field in `config.yaml` (`_read_stack_from_config`) selects the profile; `_load_stack_context()` (mirroring `_load_bet_context`) loads it and the run loop injects it per-step for engineer/reviewer/security-reviewer/automation. No `stack:` → agents run stack-neutral (honest fallback).
- **Override resolution** (`_resolve_compass_file`): `<project>/compass-overrides/<rel-path>` → Compass default. MVP wires it for stack profiles; the same resolver extends to templates/workflows (staged). *Opinionated defaults, fully overridable* — the enterprise "bring-your-own-methodology" lever; the sample runs Compass's industry-standard defaults.
- Orchestrator shell-exec unchanged (bash already runs `dotnet`).

**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **302 pass** (+4: `_read_stack_from_config` reads/strips-comments/ignores-nested/handles-absent · `_resolve_compass_file` override-wins/default/none · `_load_stack_context` loads + empty-on-missing · shipped reference profiles exist). consistency-check CONSISTENT. Straggler grep: no load-bearing stack-specifics remain in the agents.

**Files touched (9):** `compass/orchestrator/run.py` · `compass/config.yaml` · `compass/stacks/nextjs-ts.md` (new) · `compass/stacks/dotnet-blazor.md` (new) · `compass/agents/engineer.md` · `compass/agents/reviewer.md` · `compass/agents/automation.md` · `compass/orchestrator/tests/test_graph.py` (+ CHANGELOG + this file). Counter: #160. **(#030 batch: #158–#160 of #158–#167.)** Merged via PR #2. The **.NET de-risk** (one Blazor story end-to-end with `stack: dotnet-blazor`) is **deferred to the real POC environment** (no `dotnet` SDK on the dev box; DRI decision). Known follow-up: extend the override resolver from stack profiles to templates + workflows.

### 2026-06-28 — governance audit export + actor identity + cross-model provenance (#161)

**Trigger origin (MVP plan):** control-tower MVP capability #2a — the *provenance half* of the SOW-conformance wedge. The N=1 sale is "prove every control is met, with evidence"; the audit explore (during planning) found the data exists (spine + `hitl.jsonl` + `runs.jsonl` capture run lifecycle, host/model per step, gate decisions) but there was **no human-actor identity, no cross-model independence as an asserted fact, and no export** — only stdout tables.
...
**Files touched (4):** `compass/templates/controls.md` (new) · `compass/orchestrator/logger.py` · `compass/orchestrator/run.py` · `compass/orchestrator/tests/test_jsonl_pipeline.py` (+ CHANGELOG + this file). Counter: #162. **(#030 batch: #158–#162 of #158–#167.)** Merged via PR #4. MVP capability #2 COMPLETE.

### 2026-06-28 — Compass→Jira/Confluence one-way projection (#163)

**Trigger origin (MVP plan):** capability #4 — "works in their tools." The deliverables are Compass-primary; this projects them one-way into the team's Atlassian so they show up where the delivery team actually works (decided toolchain: GitHub + Jira + Confluence). The connector contract + the gate-approval call site already existed (only `filesystem` was implemented, with honest fallback) — this fills in the two backends.

**What shipped.**
- **`stores.py`** — `jira_push()` (issue create POST / update PUT, ADF description) + `confluence_push()` (page create POST / update PUT with a version-bump GET), authed server-to-server from the **environment** (`JIRA_*` / `CONFLUENCE_*`, `ATLASSIAN_*` shared fallback — nothing committed; reachable headless). The HTTP transport is **injectable** (`transport=`), default urllib (stdlib — no new dep).
- **`connector.push_artifact()` routes** to the backends: it always writes the **Compass-primary filesystem cache**, then projects one-way when credentialed, storing the **distribution pointer** (`jira_key` / `confluence_page_id`) on the artifact frontmatter so a re-push is an **idempotent update-not-create** (load-bearing now that #158 creates the whole backlog at once). Missing creds / failure → honest filesystem-fallback label (hitl.jsonl never lies). New helpers `_frontmatter_field` / `_set_frontmatter_field` / `_artifact_title`.
- **`resolve_connector_for_artifact()`** routes per artifact type — a `story.md` → ticketing (jira); foundation / brief / architecture docs → docs (confluence) — generalizing `resolve_connector` (now `_config_connector(key)`). The `/build` gate-approval promotion uses it.
- **config.yaml** documents the projection auth env vars (secret-safe). [declare-not-implement] graduates: jira + confluence are now in `IMPLEMENTED_BACKENDS`.

**Why it lands the demo beat.** Approving a gate now creates/updates a real Jira issue + Confluence page (when credentialed) — the delivery team sees Compass driving their own board, with Compass canonical underneath. The injectable transport means the mapping + idempotency are proven in CI without live creds; an adopter sets the env vars and it's live.
...
- **(2) Gate-resume didn't promote.** A `--from-step` resume straight to the HITL gate left `last_agent_output = ""` (the prior agent step was loaded from disk, not re-run), so approval hit `elif not last_agent_output:` → *"no prior step output to promote"* → the artifact never flipped to approved or projected (architecture.md stayed `proposed` after the human approved). Fix: seed `last_agent_output` from the last loaded prior output on resume; `_promote_artifact` (#166) then reads the on-disk artifact for the real content.
- **(3) False `AUTHORING INCOMPLETE` on the delivery-manager step.** It *wrote* `status.md` (4 successful edits) but its summary said *"WLT-27 awaiting human approval"* — and the #159 classifier regex matched bare `…approval`, marking the step ✗ and tripping `AUTHORING INCOMPLETE`. Tightened the #159 additions to require **"permission"** context (not bare "approval"), so a status report that legitimately narrates awaiting-states isn't flagged — while the real permission-block phrases ("waiting on permission approval", "permission dialog", "approve both writes") still classify ✗.

**The confound (not bugs):** the corrupted `brief.md` body (orchestrator step output) + the agent-set `approved: 2026-06-29` self-approval are **home-app running a stale Compass copy**. Latest `main` already addresses them — #159 (authoring writes), #166 (promote reads the on-disk clean artifact), #153 (self-approval revert at the gate). **Action: update home-app's `compass/` to latest**, then re-run `/create-brief WLT-27` to get a clean brief.

**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **341 pass** (+4: "awaiting human approval"/"pending approval"/"awaiting your decision" classify done · permission-blocks still incomplete after the tightening · repo-reading agents exclude chatgpt). consistency-check CONSISTENT (host list intact — chatgpt still used by delivery-manager).

**Files touched (5):** `compass/agents/designer.md` · `compass/agents/ux-writer.md` · `compass/orchestrator/run.py` · `compass/orchestrator/tests/test_graph.py` (+ CHANGELOG + this file). Counter: #168. **(#031 batch opens: #168–#177.)** Merged via PR #10. Retro #030 still owed (#158–#167).

### 2026-06-29 — CI runs honest-green: host-readiness tests skip when the optional SDK is absent (#169)

**Trigger origin (consumer signal — PR #10 wouldn't merge):** the DRI hit *"#10 says cannot merge."* Diagnosis: the **Consistency Check CI had been red on every commit this session — including `main`** — and it was unrelated to any of the changes. The workflow (`.github/workflows/consistency-check.yml`) runs `python -m unittest discover` in a **bare runner with no LLM SDKs installed**, so two host-*readiness* tests hard-failed: `test_adapter_importable` (asserts `_adapter_importable("claude")` — needs `anthropic`) and `test_select_host_ready_needs_key_and_pkg` (needs the pkg present for `select_host` to return `claude`). They pass locally (SDKs installed) and were merged through all session via non-blocking `gh pr merge` — but the perpetual false-red is exactly the *"the board can't lie"* sin the framework preaches, and it surfaced as a merge-block.

**The fix (honest degradation, not "install everything"):** the host SDKs are *optional* — the orchestrator already degrades honestly when a host's package/key is absent. The tests now mirror that: a module-level `_HAS_ANTHROPIC = importlib.util.find_spec("anthropic") is not None` gates the SDK-specific assertions — `test_adapter_importable` keeps its always-true pass-through assertion and only checks the `claude` adapter when anthropic is present; `test_select_host_ready_needs_key_and_pkg` is `@skipUnless(_HAS_ANTHROPIC)`. Run fully in a dev env; skip cleanly in CI. (Chose skip over `pip install anthropic` in CI to keep the framework dependency-light and to test the real honest-degradation posture, not a propped-up env.)
...
### 2026-06-29 — design & copy become human-owned stories, not sidecar files (#171)

**Trigger origin (DRI direction):** *"to make it enterprisey — we should not create the design files but we should create stories for the design and copy, and the output of the design will be in the story."* Refined live: *"design cannot be created through AI yet, at least at enterprises — we will still wait for a designer to create the Figma and copy"* + (clarifications) copy is *"human writes it (placeholder, like design)"* and the AI role is *"AI specs the requirements; human produces the deliverable."*
...
A non-UI slice stays a single feature story (unchanged). AI never fabricates a Figma link or writes the final copy, and never flips a design/copy story past its `needs-*` status — those are HITL deliverables.

**The testable code change — WBS (`wbs.py`).** `build_wbs` now reads each story's `dependencies` + `owner` and resolves cross-story state: a feature story with a design/copy dependency still in `needs-design`/`needs-copy` surfaces **red** under manage-by-exception (*"story WLT-27-1 blocked by WLT-27-2 (needs-design)"*), the block **clears** once the human flips the dep to `ready`, and a human-owned design/copy story still awaiting delivery surfaces **amber** (*"design story … awaiting human delivery"*). `render_wbs` tags human-owned stories `(design, human)` and shows `⛔ blocked by …`.

**The 'always block' gate is mechanical (DRI decision, this session).** Asked *"if we move to designer/ux-writer story creation, will the flow stop?"*, the DRI chose **always block + show in tower** over advisory/track-only. So the refusal can't be a soft agent instruction the LLM might skip — `run.py` enforces it **before dispatching the engineer**: a story-scoped `/build` of a feature whose `dependencies` include a **design/copy-type story not yet `ready`** halts (exit 3) with the blocker named, mirroring #153's *"HITL enforced mechanically."* Crucially, **only design/copy-TYPE deps block** (`_story_human_deliverable_blockers` checks the dep's `type`, not just status) — a plain feature-ordering dependency never false-blocks, so old-shape stories predating #171 (e.g. the in-flight WLT-27 set, deps empty/feature-only) build unaffected. The flow doesn't *globally* stop: #172 makes builds per-story, so only the specific un-delivered UI feature waits; the human flips `needs-*` → `ready` and re-runs `/build` to proceed.

**Reused for free:** the connector regex `/stories/[^/]+/story\.md$` already routes **every** story to ticketing — design/copy/feature all project as Jira tickets with **no connector change**. That's the payoff of (1): design/copy are now first-class board items.

**Honest scope:** AI does NOT generate the Figma/design or the copy (the whole point — humans deliver). `/build` is not routed by story type (the rejected "option 2" — design/copy aren't AI-built; they're human deliverables the engineer waits on). Auto-detecting human delivery is out of scope — the human flips `needs-*` → `ready` (a future Jira-sync/webhook could automate the flip). **Forward-only:** the in-flight WLT-27 run used the old sidecar model; re-running `/create-story` after this lands produces the trio shape.

**Verification:** `python3 -m unittest discover -s compass/orchestrator/tests` → **359 pass** (+10: `TestStoryDependencies` ×5 — feature blocked by un-delivered design+copy; block clears when both `ready`; human-owned awaiting-delivery is amber; owner/type surface on the node; render shows `(design, human)` + `⛔ blocked by`; plus `TestStoryScoping` ×5 for the mechanical gate — dependency parsing inline+block forms; blocked when a design dep isn't ready; unblocked when both ready; a feature-ordering dep does NOT block; no-deps never blocks). consistency-check CONSISTENT. Dry-run proof on the real WLT-27-2 (old-shape, empty deps) → **not** blocked, confirming no false-block of in-flight stories.

**Files touched (11):** `compass/templates/story.md` · `compass/templates/pr-template.md` · `compass/agents/pm.md` · `compass/agents/designer.md` · `compass/agents/ux-writer.md` · `compass/agents/engineer.md` · `compass/workflows/create-story.md` (→ v0.3.57) · `compass/orchestrator/wbs.py` · `compass/orchestrator/run.py` · `compass/orchestrator/tests/test_wbs.py` · `compass/orchestrator/tests/test_gates.py` (+ CHANGELOG + this file). Counter: #171. **(#031 batch: #168–#172 of #168–#177.)** On branch `mvp/design-copy-stories` → PR #14, merged. Cross-model review (Codex/Gemini, not Claude). *(Deprecated `compass/roles/{designer,ux-writer}.md` still name the sidecars — grace-period, removed in v0.4; the migrated `compass/agents/` are authoritative.)*

### 2026-06-29 — build branches no longer stack on each other: clean fresh-base isolation (#173)

**Trigger origin (live consumer signal):** right after #172 shipped story-scoped builds, the DRI ran `/build` for WLT-27-2 then -3 and got **cumulative, conflicting PRs**: PR #125 (WLT-27-2) contained PR #124's commits; the WLT-27-3 PR contained both. *"There is an issue with the PRs."* The promise of #172 — independent, parallel story builds — was silently broken: each build was **stacking** on the previous one.

**Root cause.** `_ensure_work_branch` is meant to branch from a clean `origin/main` (#143), but `git checkout -b <branch> origin/main` **fails on a dirty working tree**, and the code then **silently fell back to `checkout -b` from the current HEAD** — i.e. the previous build's branch. The tree is *always* dirty during dogfood: the orchestrator's own telemetry (`docs/orchestrator-runs/runs.jsonl` + `hitl.jsonl`, tracked) is rewritten every run, plus each build leaves uncommitted test files. So every sequential `/build` degraded "fresh base" → "stack on the last branch," and #172 (rapid story builds in one shared tree) made it fire every time. A second bug rode along: the work branch was slugged from the **loaded bet/story context blob**, producing `feat/WLT-27-bet-context-wlt-27-briefmd-----id` (the #157 garbage-branch fix had only patched the *resume* path; fresh runs still slugged the blob).

**Fix.** (1) When the clean checkout is blocked by a dirty tree, `_ensure_work_branch` now **stashes the residue (`git stash push --include-untracked`) and retries the checkout off the fresh base** — the work branch starts clean, the stash is recoverable (`git stash list`), nothing is discarded, and the stash is deliberately **not popped** onto the new branch (that would re-introduce the contamination). The last-resort current-HEAD fallback (no remote/base) now **warns loudly** to stderr instead of stacking silently. (2) Branch names are slugged from the **original `--context`** (captured before the bet/story/full-project blobs are prepended), not the blob — a build with no context yields a clean `feat/<id>-work`.

**Honest scope.** This stops the contamination at the source (sequential builds now isolate). It is NOT yet full **git-worktree isolation** — genuinely *concurrent* builds in one checkout could still race the shared index; worktree-per-build is the durable follow-up (matches the Agent tool's `isolation: worktree`). Two cleanliness follow-ups remain: **gitignore/relocate the per-project telemetry** so it stops dirtying the tree (the stash handles it, but the tree shouldn't be perpetually dirty), and **keep doc artifacts out of code PRs** (the engineer's `git add -A` swept briefs/design/copy + a literal `<story-id>` placeholder dir into the product PRs). Cleanup done this session: stopped the runaway WLT-27-3 build, closed the 3 contaminated PRs (#124/#125/#126), reaped the killed run from the spine.
...
**Files touched (2):** `compass/orchestrator/run.py` · `compass/orchestrator/tests/test_gates.py` (+ CHANGELOG + this file). Counter: #174. **(#031 batch: #168–#174 of #168–#177.)** On branch `feat/worktree-isolation` → PR #16, merged. Cross-model review (Codex/Gemini, not Claude).

### 2026-06-29 — post-worktree cleanup: telemetry out of git + worktree pruning (#175)

**Trigger origin (DRI direction).** *"Yes merge and clean up"* — the two follow-ups flagged after #174: stop the per-project telemetry dirtying the tree, and bound the worktree accumulation. (The third — doc artifacts in code PRs — is largely subsumed: the telemetry `.gitignore` removes the jsonl noise, and #174's worktree branches off main where story docs are already committed, so they aren't re-staged.)

**Root the #173/#174 fixes left exposed.** The orchestrator writes `docs/orchestrator-runs/{runs,hitl}.jsonl` + `step-*.md` every run. In home-app these were **tracked + committed**, so every run rewrote them → the working tree was *perpetually dirty* → fresh-base branch isolation kept tripping into the stash path, and the engineer's `git add -A` swept telemetry into code PRs. The durable event spine already lives in `~/.compass`; the per-project copy is local convenience that should never be committed.
...
### 2026-06-30 — on-demand projection to Jira/Confluence: --push / --push-bet (#181, toward #34)

**Trigger origin (DRI direction).** *"Lets focus on seeing product brief, and other product related docs created in Confluence and Jira."* The projection code (#163/#166) existed but only fired on a HITL gate approval *inside* a workflow — so an already-approved brief couldn't be pushed on demand; \`--approve\` flips status but does NOT project. No way to just "show me the docs in Atlassian."

**Fix.** Two no-workflow CLI entries (mirroring \`--wbs\`/\`--export-audit\`): \`--push <path>\` projects one artifact via its configured connector (story → Jira, brief/architecture/research → Confluence, per \`config.yaml\`); \`--push-bet <BET>\` projects the whole bet's product docs (brief + architecture + research → Confluence; every story → Jira). Thin wrappers over the existing \`resolve_connector_for_artifact\` + \`push_artifact\` — env-credentialed (\`JIRA_*\`/\`CONFLUENCE_*\`/\`ATLASSIAN_*\`), honest filesystem-fallback when uncredentialed (per-artifact labels + a "set these env vars" hint; no silent no-op), idempotent re-push via the stored \`jira_key\`/\`confluence_page_id\` pointer.

**Verification:** \`python3 -m unittest discover -s compass/orchestrator/tests\` → **378 pass** (+6: push-failure labels surface HTTP status + API message; \`_bet_doc_paths\` order; story → jira routing; brief → confluence routing; missing-file ERROR — all via the no-creds fallback path so they're deterministic). consistency-check CONSISTENT. Live demo on home-app WLT-27 routed all 10 artifacts correctly (Confluence for docs, Jira for stories), falling back honestly with no creds set.

**Files touched (3):** \`compass/orchestrator/run.py\` · \`compass/orchestrator/tests/test_stores.py\` (+ CHANGELOG + this file). Counter: #181. **(#032 batch: #178–#181 of #178–#187.)** **Part of milestone-1.0 issue #34** (live Jira/Confluence) — does NOT close it; #34 closes when proven live on the pilot's real Atlassian. On branch \`feat/on-demand-push\` → PR; cross-model review (Codex/Gemini, not Claude). *(Also re-surfaced the stray \`<story-id>\` placeholder dir → issue #32.)*
