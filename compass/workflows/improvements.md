...
### 2026-06-08 — Engineer could pull external packages without Architect approval; supply-chain surface unvetted (v0.3.17)

**Friction:** The foundational-stack deviation gate (`/create-bet-architecture` step 7, shipped v0.2.4) catches *major* dependencies — tools, services, frameworks, data stores, runtimes — at architecture time and routes them to a foundational amend. But it has a hole the v0.2.4 watch-for note explicitly anticipated: *"if users start trying to characterize legitimate new dependencies as 'not really tooling.'"* At **build time**, nothing stopped the Engineer from running `pnpm add <pkg>` / `npm i` / `pip install` for a small utility library that falls below the "major dependency" threshold. The package enters the supply chain with **no evaluation of maintenance health, license, CVE exposure, or transitive footprint** — and no record that anyone decided it was OK. User surfaced the gap crisply: *"we should not allow engineering to download any package without approval — architect has to empirically approve the external packages."* The load-bearing words are **any** (not just major deps) and **empirically** (evaluate, don't rubber-stamp).

**Change:**

- **`[external-package-gate]` codified** (`compass/framework/canon.md`) as an application of `[refuse-escalate]` (#16) + cite-or-mark-`n/a` (#15) + `[soft-spec-hardening]` (#14): the Engineer may not add a NEW external package without the Architect's **empirical** approval first; dependencies are an Architect-owned upstream decision. Named anti-pattern **`unvetted-dependency`** (install-first-rationalize-later, or recharacterizing a real dependency as "not really a package" to slip the gate). Framed as a 1-instance codification candidate (build-time complement to an architecture-time gate); promote to standalone pattern on a 2nd structurally-similar instance.
- **`compass/roles/architect.md` — new "External-package empirical approval" section.** Defines the **6-category empirical evaluation** (necessity/duplication · maintenance health · security posture · license compatibility · transitive footprint/cost · provenance/adoption), each cite-or-mark-`n/a`-with-reason per principle #15 — **rubber-stamping fails the gate.** Spells out the deviation-gate-vs-package-gate threshold (major dep → foundational amend + ADR; minor package → this evaluation + ledger). Approval recorded twice: DRI Decision (reasoning) + `dependency_policy.approved_packages` ledger entry (receipt). New role-trigger, DoD assertion, and `rubber-stamping` anti-pattern added.
- **`compass/agents/engineer.md`** — `[external-package-gate]` inlined as a core principle; new work-step **4a** (stop-and-escalate before any new install); refusal rule; DoD item ("No unvetted dependency"); `unvetted-dependency` anti-pattern; DRI note that new packages are NOT an Engineer decision.
- **`compass/workflows/build.md` Phase 2** — external-package gate as a load-bearing step body with the `unvetted-dependency` anti-pattern + principle-#14 framing; "Discipline always" note that the gate holds under deadline pressure.
- **`compass/workflows/create-bet-architecture.md` step 7** — scope note connecting the deviation gate (major deps, arch time) to the build-time package gate (every package), so the two read as complements, not duplicates.
- **`compass/workflows/ops.md` Phase 3** — note that a NEW package introduced via `/ops` takes the same empirical-approval path (upgrades of already-approved packages stay on the normal dependency-upgrade path; adding a *new* dependency gets no hygiene fast path).
- **`compass/config.yaml` — new `dependency_policy:` block** (`require_architect_approval: true` hard gate + `approved_packages: []` ledger with documented entry shape). The ledger is the canonical approval record + the mechanical thing the scanner checks ("configuration as data," principle #6).
- **`compass/workflows/scan.md` — new `BUILD-08`** (Unvetted external dependency: package in manifest/lockfile with no matching ledger approval and not grandfathered). High, suppressible only with the Architect's recorded 6-category approval as rationale — gives principle #14's "mechanical verification gate" element.
- **`AGENTS.md`** — principle #15 instance list gains the Architect 6-category package evaluation; principle #16 instance list gains the Engineer-install → Architect-approval escalation.

**Files touched:** `compass/framework/canon.md`, `compass/roles/architect.md`, `compass/agents/engineer.md`, `compass/workflows/build.md`, `compass/workflows/create-bet-architecture.md`, `compass/workflows/ops.md`, `compass/config.yaml`, `compass/workflows/scan.md`, `AGENTS.md`, `CHANGELOG.md` (0.3.17), `compass/workflows/improvements.md` (this entry).

**Watch for:**

- **Ledger backfill friction on existing projects.** `BUILD-08` only stays quiet if pre-existing manifest deps are grandfathered or backfilled into `approved_packages`. On a repo with 200 existing deps, a literal scan would flood findings. The scanner should treat deps present *before* the gate's adoption commit as grandfathered (or offer a one-time bulk-backfill). If teams hit noise, add a `dependency_policy.grandfather_before: <commit/date>` knob.
- **"Empirically" can still erode to rubber-stamp.** The 6-category cite-or-`n/a` is the structural defense, but an Architect under pressure can write `n/a` six times. If approvals start coming back all-`n/a`, that's the soft-spec surface reopening — tighten with a non-suppressible category (security posture: CVE check is never `n/a`).
- **Lockfile-only / transitive additions.** A direct dep the Architect approves can pull a transitive package nobody evaluated. This gate covers *direct* additions; transitive supply-chain risk is a deeper problem (lockfile diff review, `npm audit` in CI). Out of scope for this round — note it if a transitive CVE bites.
- **Monorepo / multi-manifest repos.** `approved_packages` is currently a flat list. Multi-package monorepos may need per-workspace ledgers. Defer until a monorepo project hits it.
- **The gate is a hard rule, not configurable off** (mirroring the deviation gate and the reviewer-split — discipline holds). `require_architect_approval: true` is documented as "do not flip without a DRI decision." If a team genuinely needs to disable it, that's a logged decision, not a silent config edit.
- **Counter at #34. Hard line still in effect from Retro #006** — Retro #007 fires after #35. **1 more improvement needed.**
